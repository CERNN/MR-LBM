    ...
    dfloat *fMom)
{
    ...
    fMom[idxMom(tx, ty, tz, 4, bx, by, bz)] = meq[1][1];
    fMom[idxMom(tx, ty, tz, 5, bx, by, bz)] = meq[1][2];
    fMom[idxMom(tx, ty, tz, 6, bx, by, bz)] = meq[1][3];
    fMom[idxMom(tx, ty, tz, 7, bx, by, bz)] = meq[2][2];
    fMom[idxMom(tx, ty, tz, 8, bx, by, bz)] = meq[2][3];
    fMom[idxMom(tx, ty, tz, 9, bx, by, bz)] = meq[3][3];
}
    ...
    dfloat *fGhostZ_0, dfloat *fGhostZ_1)
{
    ...
    if(      ty == 0            && tx == 0              && tz == 0)             {//swb
        fGhostX_0[idxPopX(ty,tz,5,(bx+cx[ 2]+BLOCK_NX)%BLOCK_NX,(by+cy[ 2]+BLOCK_NY)%BLOCK_NY,(bz+cz[ 2]+BLOCK_NZ)%BLOCK_NZ)] = fNode[ 2];
        fGhostX_0[idxPopX(ty,tz,6,(bx+cx[ 8]+BLOCK_NX)%BLOCK_NX,(by+cy[ 8]+BLOCK_NY)%BLOCK_NY,(bz+cz[ 8]+BLOCK_NZ)%BLOCK_NZ)] = fNode[ 8];
        fGhostX_0[idxPopX(ty,tz,7,(bx+cx[10]+BLOCK_NX)%BLOCK_NX,(by+cy[10]+BLOCK_NY)%BLOCK_NY,(bz+cz[10]+BLOCK_NZ)%BLOCK_NZ)] = fNode[10];
        fGhostX_0[idxPopX(ty,tz,8,(bx+cx[14]+BLOCK_NX)%BLOCK_NX,(by+cy[14]+BLOCK_NY)%BLOCK_NY,(bz+cz[14]+BLOCK_NZ)%BLOCK_NZ)] = fNode[14];
        fGhostX_0[idxPopX(ty,tz,9,(bx+cx[16]+BLOCK_NX)%BLOCK_NX,(by+cy[16]+BLOCK_NY)%BLOCK_NY,(bz+cz[16]+BLOCK_NZ)%BLOCK_NZ)] = fNode[16];
        
        fGhostY_0[idxPopY(tx,tz,5,(bx+cx[ 4]+BLOCK_NX)%BLOCK_NX,(by+cy[ 4]+BLOCK_NY)%BLOCK_NY,(bz+cz[ 4]+BLOCK_NZ)%BLOCK_NZ)] = fNode[ 4];
        fGhostY_0[idxPopY(tx,tz,6,(bx+cx[ 8]+BLOCK_NX)%BLOCK_NX,(by+cy[ 8]+BLOCK_NY)%BLOCK_NY,(bz+cz[ 8]+BLOCK_NZ)%BLOCK_NZ)] = fNode[ 8];
        fGhostY_0[idxPopY(tx,tz,7,(bx+cx[12]+BLOCK_NX)%BLOCK_NX,(by+cy[12]+BLOCK_NY)%BLOCK_NY,(bz+cz[12]+BLOCK_NZ)%BLOCK_NZ)] = fNode[12];
        fGhostY_0[idxPopY(tx,tz,8,(bx+cx[13]+BLOCK_NX)%BLOCK_NX,(by+cy[13]+BLOCK_NY)%BLOCK_NY,(bz+cz[13]+BLOCK_NZ)%BLOCK_NZ)] = fNode[13];
        fGhostY_0[idxPopY(tx,tz,9,(bx+cx[18]+BLOCK_NX)%BLOCK_NX,(by+cy[18]+BLOCK_NY)%BLOCK_NY,(bz+cz[18]+BLOCK_NZ)%BLOCK_NZ)] = fNode[18];
...
/* ------------------------------ FACE ------------------------------ */
    ...
    if (ty == 0)
    { //s
        fGhostY_0[idxPopY(tx, tz, 0, (bx), (by + cy[4] + BLOCK_NY) % BLOCK_NY, (bz))] = fNode[4];
        fGhostY_0[idxPopY(tx, tz, 1, (bx), (by + cy[8] + BLOCK_NY) % BLOCK_NY, (bz))] = fNode[8];
        fGhostY_0[idxPopY(tx, tz, 2, (bx), (by + cy[12] + BLOCK_NY) % BLOCK_NY, (bz))] = fNode[12];
        fGhostY_0[idxPopY(tx, tz, 3, (bx), (by + cy[13] + BLOCK_NY) % BLOCK_NY, (bz))] = fNode[13];
        fGhostY_0[idxPopY(tx, tz, 4, (bx), (by + cy[18] + BLOCK_NY) % BLOCK_NY, (bz))] = fNode[18];
    }
    else if (ty == (BLOCK_NY - 1))
    { //n
        fGhostY_1[idxPopY(tx, tz, 0, (bx), (by + cy[3] + BLOCK_NY) % BLOCK_NY, (bz))] = fNode[3];
        fGhostY_1[idxPopY(tx, tz, 1, (bx), (by + cy[7] + BLOCK_NY) % BLOCK_NY, (bz))] = fNode[7];
        fGhostY_1[idxPopY(tx, tz, 2, (bx), (by + cy[11] + BLOCK_NY) % BLOCK_NY, (bz))] = fNode[11];
        fGhostY_1[idxPopY(tx, tz, 3, (bx), (by + cy[14] + BLOCK_NY) % BLOCK_NY, (bz))] = fNode[14];
        fGhostY_1[idxPopY(tx, tz, 4, (bx), (by + cy[17] + BLOCK_NY) % BLOCK_NY, (bz))] = fNode[17];
    }
    else if (tx == 0)
    { //w
        fGhostX_0[idxPopX(ty, tz, 0, (bx + cx[2] + BLOCK_NX) % BLOCK_NX, (by), (bz))] = fNode[2];
        fGhostX_0[idxPopX(ty, tz, 1, (bx + cx[8] + BLOCK_NX) % BLOCK_NX, (by), (bz))] = fNode[8];
        fGhostX_0[idxPopX(ty, tz, 2, (bx + cx[10] + BLOCK_NX) % BLOCK_NX, (by), (bz))] = fNode[10];
        fGhostX_0[idxPopX(ty, tz, 3, (bx + cx[14] + BLOCK_NX) % BLOCK_NX, (by), (bz))] = fNode[14];
        fGhostX_0[idxPopX(ty, tz, 4, (bx + cx[16] + BLOCK_NX) % BLOCK_NX, (by), (bz))] = fNode[16];
    }
    else if (tx == (BLOCK_NX - 1))
    { //e
        fGhostX_1[idxPopX(ty, tz, 0, (bx + cx[1] + BLOCK_NX) % BLOCK_NX, (by), (bz))] = fNode[1];
        fGhostX_1[idxPopX(ty, tz, 1, (bx + cx[7] + BLOCK_NX) % BLOCK_NX, (by), (bz))] = fNode[7];
        fGhostX_1[idxPopX(ty, tz, 2, (bx + cx[9] + BLOCK_NX) % BLOCK_NX, (by), (bz))] = fNode[9];
        fGhostX_1[idxPopX(ty, tz, 3, (bx + cx[13] + BLOCK_NX) % BLOCK_NX, (by), (bz))] = fNode[13];
        fGhostX_1[idxPopX(ty, tz, 4, (bx + cx[15] + BLOCK_NX) % BLOCK_NX, (by), (bz))] = fNode[15];
    }
    else if (tz == 0)
    { //b
        fGhostZ_0[idxPopZ(tx, ty, 0, (bx), (by), (bz + cz[6] + BLOCK_NZ) % BLOCK_NZ)] = fNode[6];
        fGhostZ_0[idxPopZ(tx, ty, 1, (bx), (by), (bz + cz[10] + BLOCK_NZ) % BLOCK_NZ)] = fNode[10];
        fGhostZ_0[idxPopZ(tx, ty, 2, (bx), (by), (bz + cz[12] + BLOCK_NZ) % BLOCK_NZ)] = fNode[12];
        fGhostZ_0[idxPopZ(tx, ty, 3, (bx), (by), (bz + cz[15] + BLOCK_NZ) % BLOCK_NZ)] = fNode[15];
        fGhostZ_0[idxPopZ(tx, ty, 4, (bx), (by), (bz + cz[17] + BLOCK_NZ) % BLOCK_NZ)] = fNode[17];
    }
    else if (tz == (BLOCK_NZ - 1))
    { //f
        fGhostZ_1[idxPopZ(tx, ty, 0, (bx), (by), (bz + cz[5] + BLOCK_NZ) % BLOCK_NZ)] = fNode[5];
        fGhostZ_1[idxPopZ(tx, ty, 1, (bx), (by), (bz + cz[9] + BLOCK_NZ) % BLOCK_NZ)] = fNode[9];
        fGhostZ_1[idxPopZ(tx, ty, 2, (bx), (by), (bz + cz[11] + BLOCK_NZ) % BLOCK_NZ)] = fNode[11];
        fGhostZ_1[idxPopZ(tx, ty, 3, (bx), (by), (bz + cz[16] + BLOCK_NZ) % BLOCK_NZ)] = fNode[16];
        fGhostZ_1[idxPopZ(tx, ty, 4, (bx), (by), (bz + cz[18] + BLOCK_NZ) % BLOCK_NZ)] = fNode[18];
    }
}
